 ...
 *   colors     → --lumen-*
 *   typography  → --lumen-typo-*
 *   borderRadius → --lumen-radius-*
 *   elevation   → --lumen-elevation-*
 *   spacing     → --lumen-space-*
 *   iconStyle   → --lumen-icon-style-*
 */
export function applyAllThemeVars(
  ...
  applyThemeVars(theme.colors as unknown as NestedRecord, target, '--lumen');

  // Typography
  applyThemeVars(theme.typography as unknown as NestedRecord, target, '--lumen-typo');

  // Border Radius
  applyThemeVars(theme.borderRadius as unknown as NestedRecord, target, '--lumen-radius');

  // Elevation (Shadows)
  applyThemeVars(theme.elevation as unknown as NestedRecord, target, '--lumen-elevation');

  // Spacing
  applyThemeVars(theme.spacing as unknown as NestedRecord, target, '--lumen-space');
  ...
  applyThemeVars(iconStyleVars as unknown as NestedRecord, target, '--lumen-icon-style');

  // Component Radius (per-theme semantic radius overrides)
  applyThemeVars(theme.componentRadius as unknown as NestedRecord, target, '--lumen-comp-radius');

  // Data Visualization palette
  applyThemeVars(theme.colors.dataViz as unknown as NestedRecord, target, '--lumen-dataviz');
}
