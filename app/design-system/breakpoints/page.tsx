...
export default function BreakpointsPage() {
  ...
  return (
    ...
    >
        ...
        <div style={sharedStyles.card}>
          ...
          <SpecTable
            ...
            rows={Object.entries(transitions.timing).map(([name, value]) => [
              <code key={name}>transitions.timing.{name}</code>,
              value,
            ])}
          />
        </div>
        ...
        <div style={sharedStyles.card}>
          ...
          <SpecTable
            ...
            rows={Object.entries(transitionPresets).map(([name, value]) => [
              <code key={name}>transitionPresets.{name}</code>,
              value,
      ...
      {activeTab === 'specs' && (
        <section style={sharedStyles.section}>
          <h2 style={sharedStyles.sectionTitle}>System Specifications</h2>
          <p style={sharedStyles.sectionDescription}>
            Complete token reference for breakpoints, z-index, and transitions.
          </p>

          <div style={sharedStyles.card}>
            <h3 style={sharedStyles.cardTitle}>Breakpoints</h3>
            <SpecTable
              headers={['Token', 'Value', 'Target Devices']}
              rows={[
                [<code key="xs">breakpoints.xs</code>, breakpoints.xs, 'Small phones'],
                [<code key="sm">breakpoints.sm</code>, breakpoints.sm, 'Large phones'],
                [<code key="md">breakpoints.md</code>, breakpoints.md, 'Tablets (portrait)'],
                [<code key="lg">breakpoints.lg</code>, breakpoints.lg, 'Tablets (landscape), small laptops'],
                [<code key="xl">breakpoints.xl</code>, breakpoints.xl, 'Desktops'],
                [<code key="2xl">breakpoints['2xl']</code>, breakpoints['2xl'], 'Large desktops'],
              ]}
            />
          </div>

          <div style={sharedStyles.card}>
            <h3 style={sharedStyles.cardTitle}>Z-Index Scale</h3>
            <SpecTable
              headers={['Token', 'Value', 'Use Case']}
              rows={Object.entries(zIndex).map(([name, value]) => [
                <code key={name}>zIndex.{name}</code>,
                value.toString(),
                name === 'hide' ? 'Hidden elements' :
                name === 'base' ? 'Default layer' :
                name === 'dropdown' ? 'Dropdown menus' :
          ...
          <div style={sharedStyles.card}>
            <h3 style={sharedStyles.cardTitle}>Transition Presets</h3>
            <SpecTable
              headers={['Token', 'Value', 'Use Case']}
              rows={Object.entries(transitionPresets).map(([name, value]) => [
                <code key={name}>transitionPresets.{name}</code>,
      ...
      {activeTab === 'usage' && (
        <section style={sharedStyles.section}>
          ...
          <p style={sharedStyles.sectionDescription}>
            How to import and use system tokens in your components.
          </p>
  ...
  Animated element
          ...
          <div style={sharedStyles.card}>
            ...
            <ul style={{ margin: 0, paddingLeft: '20px', color: colors.text.mediumEmphasis }}>
              ...
              <li style={{ marginBottom: '8px' }}>Use breakpoints consistently across all responsive layouts</li>
            </ul>
          </div>
        </section>
      )}
