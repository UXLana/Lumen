 ...
 *           `var(--lumen-*)` strings dynamically.  Some consumer bundlers
...
function tokensToVarRefs(obj, prefix = '--lumen') {
  const result = {};
...
function generate() {
  ...
  const colorVarRefs = tokensToVarRefs(shapes.colors, '--lumen');
  const typoVarRefs = tokensToVarRefs(shapes.typography, '--lumen-typo');
  const radiusVarRefs = tokensToVarRefs(shapes.borderRadius, '--lumen-radius');
  const elevationVarRefs = tokensToVarRefs(shapes.elevation, '--lumen-elevation');
  const spacingVarRefs = tokensToVarRefs(shapes.spacing, '--lumen-space');

  const output = `/**
 * LUMEN Design System — Pre-compiled Design Tokens
 *
